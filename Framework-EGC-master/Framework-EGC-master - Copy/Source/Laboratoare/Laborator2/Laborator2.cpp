...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		vector<VertexFormat> vertices
		{
			...
			// DONE: Complete the information for the cube
		};
	...
	// Create a mesh box using custom data
	{
		...
		Mesh *tetra = CreateMesh("tetra1", vertices, indices);
	}
	// Create a mesh box using custom data
	{
		vector<VertexFormat> vertices
		...
		Mesh *tTri = CreateMesh("ttri1", vertices, indices);
	}
	// Create a mesh box using custom data
	{
		vector<VertexFormat> vertices
		...
		{
			2, 1, 3,	// indices for first triangle
			0, 5, 6,	// indices for second triangle
			4, 7, 9,
			8, 10, 11
			// DONE: Complete indices data
		};
		Mesh *tetra = CreateMesh("tetra2", vertices, indices);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetra1"], shaders["VertexColor"], glm::vec3(0, 0, 0), glm::vec3(0.5f));

	RenderMesh(meshes["ttri1"], shaders["VertexColor"], glm::vec3(0, 1, 0), glm::vec3(0.5f));

	RenderMesh(meshes["tetra2"], shaders["VertexColor"], glm::vec3(0, 1, 1), glm::vec3(0.5f));
